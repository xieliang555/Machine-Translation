...
class Encoder(nn.Module):
	...
	def __init__(self, input_dim, emd_dim, enc_hid_dim,
		...
		self.embedding = nn.Embedding(num_embeddings = self.input_dim, embedding_dim = self.emd_dim)
		self.dropout = nn.Dropout(self.dropout)
		self.rnn = nn.GRU(input_size = self.emd_dim, hidden_size = self.enc_hid_dim, bidirectional = True)
...
class Attention(nn.Module):
	...
	def __init__(self, enc_hid_dim, dec_hid_dim, atten_dim):
		...
		self.atten = nn.Linear(self.enc_hid_dim*2+self.dec_hid_dim, self.atten_dim)
	...
	def forward(self, encoder_outputs, decoder_hidden):
		...
		energy = torch.tanh(self.atten(torch.cat((encoder_outputs, repeated_dec_hid), dim=2)))
...
class Decoder(nn.Module):
	...
	def forward(self, decoder_input, encoder_outputs, decoder_hidden):
		...
		weighted_encoder_rep = self._weighted_encoder_rep(encoder_outputs, decoder_hidden)
		...
		output = self.out(torch.cat((output, embedded, weighted_encoder_rep), dim=1))
...
class Seq2Seq(nn.Module):
	...
	def forward(self, src, trg, teacher_forcing_ratio = 0.5):
		...
		for t in range(1, max_seq_len):
			output, hidden = self.decoder(decoder_input, encoder_outputs, hidden)
			outputs[t] = output
